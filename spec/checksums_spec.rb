...
FOO_CHECKSUM = '66d3b6e55fd94f1752bc8654335d8ff4'
BAR_CHECKSUM = 'c2223b5c324e395fd9f9bb249934ac87'
module HathiTrust
...
#{BAR_CHECKSUM}  bar
eot
        expect(Checksums.new(sample).checksums).to eq ({
          'foo' => FOO_CHECKSUM,
          'bar' => BAR_CHECKSUM
        })
      end
      ...
      it "accepts an input stream from a zip file" do
        Zip::File.new(sample_zip) do |zip_file|
          zip_file.glob('**/checksum.md5').first.get_input_stream do |file_stream|
            expect(Checksums.new(file_stream).checksums).to eq({
